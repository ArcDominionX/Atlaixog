...
export const Detection: React.FC<DetectionProps> = ({ onSearch }) => {
    ...
    // Refs for positioning (still used for other filters)
    ...
    // Close filters on outside click and scroll
    useEffect(() => {
            ...
            <div className="mb-4 relative z-50">
                ...
                <div className="flex items-center gap-3 overflow-x-auto custom-scrollbar pb-3 px-1">
                    <div className="filter-wrapper relative flex-shrink-0">
                        <button 
                            className={`filter-pill ${activeFilter === 'feedChain' ? 'active' : ''}`} 
                            onClick={() => toggleFilter('feedChain')}
                            ref={el => (buttonRefs.current['feedChain'] = el)}
                        >
                    ...
                    <div className="filter-wrapper relative flex-shrink-0">
                        <button 
                            className={`filter-pill ${activeFilter === 'event' ? 'active' : ''}`} 
                            onClick={() => toggleFilter('event')}
                            ref={el => (buttonRefs.current['event'] = el)}
                        >
                    ...
                    <div className="filter-wrapper relative flex-shrink-0">
                        <button 
                            ...
                            onClick={() => toggleFilter('timerange')}
                        >
                        ...
                        {activeDateInput ? (
                            <div className="flex flex-col items-center">
                                <div className="w-full flex justify-between items-center mb-4">
                                    <button 
                                        className="text-xs font-bold text-text-medium hover:text-text-light flex items-center gap-1"
                                        onClick={() => setActiveDateInput(null)}
                                    >
                                        <ChevronDown size={14} className="rotate-90" /> Back
                                    </button>
                                    <span className="text-sm font-semibold text-primary-green">
                                        Select {activeDateInput === 'from' ? 'Start' : 'End'} Date
                                    </span>
                                </div>
                                <CustomCalendar 
                                    onSelect={handleDateSelect} 
                                    onCancel={() => setActiveDateInput(null)} 
                                />
                            </div>
                        ) : (
                            /* Range Inputs View */
                            <>
                                <div className="flex gap-4 mb-6">
                                    <div className="flex-1">
                                        ...
                                        <div 
                                            className="bg-card border border-border rounded-lg p-3 cursor-pointer hover:border-text-medium transition-colors text-sm text-text-light"
                                ...
                                <div className="flex gap-3">
                                    <button 
                                        className="flex-1 py-3 bg-transparent border border-border rounded-lg font-bold text-sm hover:bg-card-hover transition-colors text-text-medium"
                                        onClick={() => setShowDateRangeModal(false)}
                                    >
                                        Cancel
                                    </button>
                                    <button 
                                        className="flex-1 py-3 bg-primary-green rounded-lg font-bold text-sm text-[#111] hover:bg-primary-green-darker transition-colors"
                                        onClick={() => setShowDateRangeModal(false)}
                                    >
                                        Apply
                                    </button>
