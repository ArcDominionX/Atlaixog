...
import { ArrowLeft, ShieldCheck, Users, TrendingUp, AlertTriangle, Activity, Zap, Search, Wallet, Bell, ChevronDown, Check, Box, Flame, AlertCircle } from 'lucide-react';
    ...
    return (
        <div className="space-y-6 pb-10 animate-fade-in">
            ...
            <div className="flex flex-col gap-4">
                <button onClick={onBack} className="flex items-center gap-2 text-text-medium hover:text-text-light transition-colors w-fit font-medium">
                    ...
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <div className="flex gap-2">
                             {['1H', '4H', '12H', '24H'].map(tf => (
                                <button key={tf} className={`px-3 py-1 text-xs font-bold rounded border transition-all ${timeFrame === tf ? 'bg-primary-green/10 text-primary-green border-primary-green/30' : 'bg-transparent border-border text-text-medium hover:text-text-light'}`} onClick={() => setTimeFrame(tf)}>
                                    {tf}
                                </button>
                             ))}
                        </div>
            ...
            {/* Event Feed (Styled like Detection page) */}
            <div>
