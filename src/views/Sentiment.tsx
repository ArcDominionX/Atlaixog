...
import { Search, TrendingUp, Users, MessageCircle, ExternalLink, ArrowLeft } from 'lucide-react';

// Declare ApexCharts since it's loaded via CDN in index.html
declare var ApexCharts: any;

interface SentimentProps {
    initialContract?: string | null;
    onAnalyze: (contract: string) => void;
    onBack: () => void;
}

export const Sentiment: React.FC<SentimentProps> = ({ initialContract, onAnalyze, onBack }) => {
    // If initialContract is passed, we are in "Analyzed" mode
    const analyzed = !!initialContract;
    ...
    useEffect(() => {
        if (analyzed && chartRef.current && typeof ApexCharts !== 'undefined') {
            ...
            if (timeFilter === '1H') { 
                ...
                d1 = categories.map(() => Math.floor(30 + Math.random() * 30)); 
                d2 = categories.map(() => Math.floor(20 + Math.random() * 20)); 
                d3 = categories.map(() => Math.floor(10 + Math.random() * 15)); 
                d4 = categories.map(() => Math.floor(5 + Math.random() * 20)); 
            ...
            const options = {
                ...
                yaxis: { 
                    show: true, 
                    labels: { style: { colors: '#8F96A3', fontFamily: 'Inter' } } 
                },
                grid: { 
                    show: true, 
                    borderColor: '#2A2E33', 
            ...
            <div className="flex flex-col items-center justify-center min-h-[60vh] p-4">
                <div className="w-full max-w-lg text-center">
                    ...
                    <div className="bg-main border border-border rounded-xl p-1 mb-4 flex items-center">
                        <input 
                            type="text" 
                            className="w-full bg-transparent p-4 text-text-light outline-none"
                    ...
                    <button 
                        className="w-full py-4 bg-primary-green text-main font-bold rounded-xl hover:bg-primary-green-darker transition-colors"
                        onClick={() => { if(contractInput) onAnalyze(contractInput); }}
                    >
                        Analyze Sentiment
                    </button>
                </div>
        ...
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-text-medium text-sm mt-1">Contract: {initialContract}</p>
                </div>
                <button 
                    className="px-4 py-2 border border-border rounded-lg text-sm font-medium hover:text-text-light hover:border-text-light transition-colors"
                    onClick={onBack}
                >
                    Change Token
                </button>
                            ...
                            {['1H', '6H', '12H', '1D', '1W', '1M'].map(tf => (
                                <button 
                                    ...
                                    className={`px-3 py-1 text-xs font-semibold rounded-full border transition-all ${
                                        timeFilter === tf 
                                        ? 'bg-card-hover border-primary-green text-primary-green' 
                                        : 'bg-transparent border-border text-text-medium hover:text-text-light hover:border-text-light'
                                    }`}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-card border border-border rounded-2xl p-6">
                    ...
                    <div className="flex flex-wrap gap-3">
                        {['Solana meme', 'Breakout soon', 'Smart money accumulation', 'Airdrop rumors', 'Whale entry'].map(k => (
                            <span key={k} className="bg-[#222529] border border-border px-4 py-2 rounded-lg text-sm text-text-light">
                                {k}
                            </span>
                        ))}
                    </div>
                ...
                <div className="bg-card border border-border rounded-2xl overflow-hidden flex flex-col">
                                    ...
                                    <tr key={i} className="border-b border-border last:border-0 hover:bg-card-hover/50">
                                        ...
                                        <td className="px-3 py-3">
                                            <span className={`px-2 py-1 rounded text-[10px] uppercase font-bold border ${
                                                k.sent === 'Bullish' 
                                                ? 'bg-[rgba(38,211,86,0.15)] text-primary-green border-[rgba(38,211,86,0.3)]' 
                                                : 'bg-[rgba(242,201,76,0.15)] text-primary-yellow border-[rgba(242,201,76,0.3)]'
                                            }`}>
                                                {k.sent}
                                            </span>
                                        </td>
                    ...
                    <div className="p-6 pt-4 mt-auto">
                        <button className="w-full py-2.5 bg-card border border-dashed border-border rounded-xl text-text-medium text-sm hover:bg-card-hover hover:text-text-light hover:border-text-light transition-all">
                            See More
                        </button>
                    </div>
                </div>
